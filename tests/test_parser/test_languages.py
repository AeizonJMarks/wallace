...
def python_file(tmp_path):
    ...
    content = '''#### META: Title: Python Test
#### META: Version: 0.1.1
...
def test_python_parsing(python_file):
    ...
    assert all(tag.content.strip().startswith(("Title:", "Version:", "PATH:")) for tag in meta_tags)
...
'''
    ...
    with pytest.raises(WallaceError) as exc:
        parse_file(str(file))
    assert "Inconsistent comment style" in str(exc.value)
